...
export default {
  ...
  mutations: {
	...
	actions: {
		...
		// 비딩디테일을 서버에 요청
		async sp_admin_get_new_bidding_details({rootState,commit}, {biddingId}){
			try {
				const res = await biddingsApi.sp_admin_get_new_bidding_details({rootState,biddingId})
		...
		// 변경된 비딩디테일을 서버에 수정요청
		async sp_admin_update_bidding_details({state, rootState}){
			try {
				await biddingsApi.sp_admin_update_bidding_details({state, rootState})
				location.reload()
			} catch (e) {
				console.log(e)
			}
