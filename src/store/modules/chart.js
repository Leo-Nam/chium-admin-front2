...
export default {
  ...
  actions: {
    ...
    async sp_admin_retrieve_stat_registeration({ rootState, commit, state }) {
      try {
        // 1월 ~ 12월까지의 마지막 날, 인덱싱을 위해 첫 월은 0으로 지정
        const lastDay = [0, 31, 28, 31, 30, 31, 30, 31, 31, 30, 31, 30, 31];
        const res = await chartApi.sp_admin_retrieve_stat_registeration({state, rootState});
    ...
    // 시도 정보 가져옴
    async sp_req_sido({ rootState, commit }) {
      try{
        const res = await chartApi.sp_req_sido({rootState})
    ...
    // 시군구 정보를 가져옴
    async sp_admin_retrieve_stat_region({ rootState, state, commit }) {
      let res
      try{
        res = await chartApi.sp_admin_retrieve_stat_region({rootState, state});
      }
      catch (e) {
        console.log(e)
      }
