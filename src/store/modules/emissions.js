...
export default {
	...
	actions: {
		...
		async sp_get_site_list_whose_biz_areas_of_interest({state, rootState, commit}){
			try {
				const bCode = state.orderInfo.KIKCD_B_CODE
				const res = await emissionsApi.sp_get_site_list_whose_biz_areas_of_interest({rootState ,bCode })
		...
		async sp_get_site_list_inside_range({state, rootState, commit}){
			try {
				const lat = state.orderInfo.LAT
				const lng = state.orderInfo.LNG
				const distance = state.circleRange
				const res = await emissionsApi.sp_get_site_list_inside_range({rootState ,distance, lat, lng})
		...
		async sp_get_prospective_site_list_inside_range({rootState,state,commit}){
			const lat = state.orderInfo.LAT
			const lng = state.orderInfo.LNG
			const distance = state.circleRange
			let isTransit
